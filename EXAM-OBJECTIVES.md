# CCAR-F — Exam Objectives

Every task statement tested on the **Claude Certified Architect – Foundations** exam, with the
knowledge and skills measured against each.

**Provenance.** This is §6 of the *Claude Certified Architect – Foundations Exam Guide*, v1.0
(effective July 2026), reproduced verbatim — extracted from the PDF and reformatted as markdown, not
retyped or paraphrased. §6 opens:

> *"Each domain below lists the task statements tested, with the knowledge and skills measured against
> each. Exam items are written against these objectives."*

That second sentence is what makes this file load-bearing.

**Two deliberate additions to the verbatim text**, both to save cross-referencing: the domain headings
...
Obtain your own copy of the guide — it is distributed through the Anthropic Partner Academy, which §11
names as the registration channel. This file is a working aid, not a replacement for reading the guide
in full.

**30 task statements** across 5 domains — D1 7 · D2 5 · D3 6 · D4 6 · D5 6. Any source claiming 27 is
wrong.

## How to use this — read before teaching

**These bullets are the scope boundary and the floor. They are never the lesson plan.**

They are terse *scope statements* written for the guide's §2 candidate — someone with 6+ months
hands-on experience. They say what is tested, not how to reason about it, and not the mechanism
underneath. Three rules follow:

1. **Teach the mechanism beneath a bullet**, not the bullet. "Fork-based session management for
   exploring divergent approaches from a shared analysis baseline" assumes you already know what a fork
   does to a session.
2. **Drill judgment above it.** This exam scores scenario judgment; bullet coverage is not readiness.
   A learner who can recite all 30 statements and cannot choose between a hook and a prompt will fail.
3. **Use them only to check nothing was skipped** — as a completeness audit at the end of a domain,
   not as the syllabus you read out at the start.

Where a section needs more than the bullets give, go beyond them. Where a bullet names something
Anthropic's own documentation does not cover — stratified sampling, crash-recovery manifests — the
guide is the only first-party source, so teach it from the bullet and say so.

**Companion files:** `RESOURCES.md` (what to read per statement, and how to probe it) ·
`TEACHING-BRIEF.md` (how to teach, how to write items, how to drill).
